...
test.describe("Search From HomePage", () => {
 ...
 test.beforeEach(async ({ page, request }) => {
   ...
   await page.goto("/");
   await home.darkModeToggle.check();
   await expect(home.darkModeToggle).toBeChecked();
 });
 ...
 test("Search by bedrooms", async ({ page }) => {
   ...
   for (let i = 0; i < count; i++) {
     const bedroomsText = await featuredListings.bedroomsRow.nth(i).textContent();
 ...
 test("Search by city", async ({ page }) => {
   ...
   const addressFromCard = (await featuredListings.addressRow.first().textContent()).trim();
   ...
   const garageFromCard = await featuredListings.getFirstListingGarage();
   const bedroomsFromCard = await featuredListings.getFirstListingBedrooms();
   const bathroomsFromCard = await featuredListings.getFirstListingBathrooms();

   const realtorRow = await featuredListings.realtor.first().textContent();
   const realtorFromCard = realtorRow.replace(/([a-z])([A-Z])/g, '$1 $2').trim();
